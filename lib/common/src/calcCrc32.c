...
#define calcCrc32_LUT_SIZE   (256U)
#define calcCrc32_POLYNOMIAL (0xEDB88320U)
#define calcCrc32_SEED       (0xFFFFFFFFU)

static bool f_crc32_initialised
    = false; /** set to true once the look up table has been populated */
static uint32_t f_crc32_lut[calcCrc32_LUT_SIZE]; /** pre-calculated bit pattern
                                                     look up table */
/*******************************************************************************
 *                         Private Functions
 ******************************************************************************/
...
{
    for ( uint32_t ix = 0; ix < calcCrc32_LUT_SIZE; ix++ ) {
        uint32_t entry = ix;
        for ( uint32_t bits = 0; bits < 8; bits++ ) {
            if ( 0 != (entry & (uint32_t)(0x1)) ) {
                entry = (entry >> 1) ^ (uint32_t)(calcCrc32_POLYNOMIAL);
            } else {
                entry = entry >> 1;
            }
        }
...
uint32_t calcCrc32_Calc(const uint8_t* i_pData, uint32_t in_length)
{
    ...
    if ( i_pData != NULL ) {
        ...
        result = (uint32_t)(calcCrc32_SEED);

        for ( uint32_t ix = 0; ix < in_length; ix++ ) {

            xor_value = (uint32_t)(0xFF) & (uint32_t)(i_pData[ix]);
            lut_ix    = result ^ xor_value;
            result    = (result >> 8) ^ f_crc32_lut[(lut_ix & 0xffU)];
        }
